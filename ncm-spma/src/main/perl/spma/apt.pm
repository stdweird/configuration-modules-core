...
sub generate_sources
{
    ...
    foreach my $source (@$sources) {
        ...
        my $rs = EDG::WP4::CCM::TextRender->new($template, $source, relpath => 'spma', log => $self);
        if ($rs) {
            my $fh = $rs->filewriter("$sources_dir/$source->{name}.list", log => $self);
...
sub get_installed_pkgs
{
    ...
    my $out = CAF::Process->new($CMD_DPKG_QUERY, keeps_state => 1, log => $self)->output();
    if ($?) {
...
# returns a list of packages formatted with name, version and architecture for use with apt
...
# Apply versions from configuration tree to a bare list of packages
...
sub upgrade_packages
{
    ...
    my $cmd = CAF::Process->new($CMD_APT_UPGRADE, log => $self);
    return $cmd->execute() ? 1 : undef;
}
...
sub install_packages
{
    ...
    my $cmd = CAF::Process->new([@$CMD_APT_INSTALL, @$packages], log => $self);
    return $cmd->execute() ? 1 : undef;
}
...
sub mark_packages_auto
{
    ...
    my $cmd = CAF::Process->new([@$CMD_APT_MARK, 'auto', @$packages], log => $self);
    return $cmd->execute() ? 1 : undef;
}
...
sub autoremove_packages
{
    ...
    my $cmd = CAF::Process->new([@$CMD_APT_AUTOREMOVE], log => $self);
    return $cmd->execute() ? 1 : undef;
}
